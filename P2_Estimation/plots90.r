...
library(ggplot2)
theme_set(theme_classic(16)) # To be changed to bw later
library(patchwork)
...
i90mash = merge(i90stack[Metric == 'Coverage' & estimate=='cir' & !grepl('slow', Framework)],
	...
	p1 <- ggplot(p90mash, aes(x=align, y=Value.y)) + geom_point(color='grey', size = 5) +
			geom_segment(aes(xend=align, yend=Value.x), arrow = arrow() , lineend='round',linejoin='round', linewidth=1.5) + ylim(0,y1max) + geom_hline(yintercept=0)  + geom_point(aes(y=Value.x), size = 5)
