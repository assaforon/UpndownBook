...
# for(a in 1:72) lines(1:12, pweib3(1:12, shp=weib30$shap[a], scl=weib30$scal[a], shift=weib30$shif[a]+weib30$offs[a]), col= mycolors36[a %% 36 + 1])
# abline(h=.9,lty=3)

#------------------------------ Logistic --------------------------#


logi30c = data.table(scal = 2^runif(buff30*2*nsim, -1, 2.),
					loc = runif(buff30*2*nsim, midtarg-0.25, midtarg * 2) )
...
# Targets: "exact..." (parametric)
logi30c[ , t30 := qlogis(0.3, scale = scal, location = loc) ]	
logi30c = logi30c[t30 %between% c(midtarg-0.5, midtarg+0.5) ]

 # ... and interpolated between closest levels (F-bar)				
logi30c[,row0 := 1:.N]
logi30c[ , t30bar := approx(c(pm1, pp1), midtarg + c(-0.5, 0.5), xout = 0.3)$y, by = 'row0']
